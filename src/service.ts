...
import log, { TypesEnum } from "@f0c1s/node-common-log-lib";
...
server.get(path, (req, res) => {
    ...
    const output = JSON.stringify(deprofane(input));
    const ts = Date.now();
    const hash = sha256(output);
    res.send(JSON.stringify({ hash, output, input, ts }));
});
/* SERVER IS READY */
const port = process.env["PORT"] || config.ports.services.deprofane;
log(
    TAGS.INFO,
    `Serving deprofane service at port ${bgblue(port)} path ${bgred(path)}.`,
);
server.listen(port);
